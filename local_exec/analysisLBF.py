...
def save_Backup(models,phishing_URLs,X_train,y_train,X_test,y_test,testing_list,verbose=True):   
  '''
  ...
  for i in range(len(h_sizes)): # Cambiato range da 3 a h_sizes
    LBF_backups[i] = {}
    for fpr in fprs:
        for fpr_ratio in fpr_ratios:
            ...
            LBFo = {"FPR": fpr0, "size": BF_size+h_sizes[i], "time": t}
...
def LBF_graph(models,phishing_URLs,X_train,y_train,falseN=True,FPR=True,Size=True,verbose=True):
  '''
  stampa grafici su LBF, i parametri false neg, FPR e totsize determinano quali grafici stampare
  '''
